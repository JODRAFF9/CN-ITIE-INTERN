# =============================================================================
# OpenRBE - Module Analyse PPE v1.3
# Informations exhaustives — Pas de mauve — NA = "Non renseigné"
...
ppeUI <- function(id) {
  ...
  shiny::tagList(
    ...
    # Graphiques ligne 1 : nationalité + région
    shiny::fluidRow(
      shiny::column(6,
    ...
    shiny::fluidRow(
      shiny::column(12,
        shiny::div(class = "chart-card",
          shiny::div(class = "chart-title",
            shiny::icon("percent"),
            " Distribution des participations des PPE (% Action Direct)"),
...
ppeServer <- function(id, filtered_data) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    # ── KPIs — couleurs ITIE uniquement (pas de mauve) ───────────────────────
    ...
    output$kpi_pct_moy_ppe <- shiny::renderUI({
      ...
      val <- if (is.nan(moy) || is.na(moy)) "—"
             else paste0(round(moy, 1), "%")
      kpi_card("fa-percent", "Part. moy. PPE",
               val, color = "#1A6B3C")
    })
    ...
    # Graphique PPE par nationalité — rouge ITIE
    output$ppe_nat_chart <- plotly::renderPlotly({
      dt <- ppe_data()
      if (nrow(dt) == 0) {
        return(plotly::plot_ly() |>
          plotly::layout(
            title = "Aucune donnée PPE",
    ...
    # Graphique PPE par région — jaune ITIE
    ...
    # Distribution des participations — bleu marine (pas de mauve)
    output$ppe_pct_chart <- plotly::renderPlotly({
      ...
      if (length(val) == 0) {
        return(plotly::plot_ly() |>
          plotly::layout(
            title = "Participations non renseignées",
            paper_bgcolor = "white", plot_bgcolor = "white"
          ))
      }
      plotly::plot_ly(
        ...
        marker = list(
          color = "#005A8E",
          line  = list(color = "#003D61", width = 0.5)
        ),
    ...
    output$ppe_table <- DT::renderDataTable({
      ...
      nr_html <- "<em style='color:#aaa;font-size:0.87em;'>Non renseigné</em>"
      na_fmt  <- function(x) {
        v <- as.character(x)
        ifelse(is.na(x) | v %in% c("NA", ""), nr_html, v)
      }
      ...
      display <- data.frame(
        ...
        "% Action"       = ifelse(
          is.na(dt$pct_action_direct),
          nr_html,
          paste0(dt$pct_action_direct, "%")
        ),
        "% Voix"         = ifelse(
          is.na(dt$pct_voix_direct),
          nr_html,
          paste0(dt$pct_voix_direct, "%")
        ),
