...
async function doLogin(){
  ...
  if(!email||!pwd){msg.className='auth-msg err';msg.textContent='Remplissez tous les champs.';return;}
...
async function doRegister(){
  ...
  if(!prenom||!nom||!email||!pwd){msg.className='auth-msg err';msg.textContent='Remplissez tous les champs obligatoires.';return;}
  if(pwd.length<6){msg.className='auth-msg err';msg.textContent='Mot de passe trop court (6 caractères min).';return;}
...
/* INIT */
openIDB().then(()=>{loadSession();popF();applyF();buildNet();});
"""
