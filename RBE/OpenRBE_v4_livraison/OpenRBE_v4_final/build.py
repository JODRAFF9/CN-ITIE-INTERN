...
.btn-login{background:rgba(255,255,255,.15);color:#fff;border:1.5px solid rgba(255,255,255,.5);padding:6px 16px;border-radius:20px;font-family:var(--font);font-size:.8rem;font-weight:600;cursor:pointer;transition:.2s;display:flex;align-items:center;gap:6px;backdrop-filter:blur(4px)}
.btn-login:hover{background:rgba(255,255,255,.28);border-color:#fff}
.btn-login.logged{background:rgba(255,255,255,.22);border-color:var(--y);color:var(--y)}
...
.bxl{background:#1D6F42;color:#fff}.bxl:hover{background:#155232;transform:translateY(-1px)}
...
/* ── MODAL AUTH ── */
#modal-auth .modal-box{max-width:400px}
.auth-tabs{display:flex;gap:0;margin-bottom:20px;border-radius:6px;overflow:hidden;border:1px solid var(--brd)}
.auth-tab{flex:1;padding:9px;background:var(--bg);border:none;font-family:var(--font);font-size:.85rem;font-weight:600;cursor:pointer;transition:.2s;color:var(--gray)}
...
.auth-msg.ok{background:#d4edda;color:#155724;display:block}
...
.lock-overlay{display:none;position:absolute;inset:0;background:rgba(248,249,250,.92);z-index:10;border-radius:var(--radius);align-items:center;justify-content:center;flex-direction:column;gap:10px;text-align:center}
.lock-overlay.show{display:flex}
.lock-overlay i{font-size:2.5rem;color:var(--gray)}
.lock-overlay p{color:var(--gray);font-size:.88rem;max-width:240px}
.lock-overlay button{background:var(--g);color:#fff;border:none;padding:8px 20px;border-radius:4px;font-family:var(--font);font-size:.84rem;font-weight:600;cursor:pointer;transition:.2s}
.lock-overlay button:hover{background:var(--gd)}
...
<div id="hdr">
  <div class="hi">
    ...
    <div class="hdr-right">
      ...
      <button class="btn-login" id="btn-login" onclick="openAuth()">
        <i class="fas fa-user-circle"></i>
        <span id="btn-login-txt">Connexion</span>
      </button>
    </div>
  </div>
...
<div class="page active" id="page-dashboard">
  ...
  <div class="card" style="position:relative">
    ...
    <div id="lock-top" class="lock-overlay">
      <i class="fas fa-lock"></i>
      <p>Connectez-vous pour exporter les données</p>
...
<div class="page" id="page-database">
  <div class="card" style="position:relative">
    ...
    <div id="lock-db" class="lock-overlay">
      <i class="fas fa-lock"></i>
      <p>Connectez-vous pour exporter les données</p>
      <button onclick="openAuth()">Se connecter</button>
    </div>
...
<div class="page" id="page-search">
  <div class="shero">
    ...
    <div class="sw"><input type="text" id="si" placeholder="Rechercher une entreprise enregistrée..."><div id="sdd"></div></div>
  </div>
  <div id="ccw"></div>
</div>
...
<div class="page" id="page-ppe">
  ...
  <div class="card" style="position:relative">
    ...
    <div id="lock-ppe" class="lock-overlay">
      <i class="fas fa-lock"></i><p>Connectez-vous pour exporter les données</p>
      <button onclick="openAuth()">Se connecter</button>
    </div>
...
<!-- ══ MODAL AUTH ══ -->
<div class="modal-overlay hidden" id="modal-auth" onclick="closeOnBg(event,'modal-auth')">
  <div class="modal-box" style="max-width:400px">
    <div class="modal-hdr">
      <h3><i class="fas fa-user-circle"></i> <span id="auth-title">Connexion</span></h3>
    ...
    <div class="modal-body">
      <div class="auth-tabs">
        ...
        <button class="auth-tab" id="tab-register" onclick="switchAuth('register')">Inscription</button>
      </div>
      <!-- LOGIN -->
      <div id="pane-login">
        <div class="form-group"><label>Adresse e-mail</label><input type="email" id="l-email" placeholder="votre@email.com" autocomplete="email"></div>
        <div class="form-group"><label>Mot de passe</label><input type="password" id="l-pwd" placeholder="••••••••" autocomplete="current-password"></div>
        <button class="btn-primary" onclick="doLogin()"><i class="fas fa-sign-in-alt"></i> Se connecter</button>
        <div class="auth-msg" id="l-msg"></div>
      </div>
      <!-- REGISTER -->
      <div id="pane-register" style="display:none">
        <div class="form-group"><label>Prénom</label><input type="text" id="r-prenom" placeholder="Votre prénom" autocomplete="given-name"></div>
        <div class="form-group"><label>Nom</label><input type="text" id="r-nom" placeholder="Votre nom" autocomplete="family-name"></div>
        <div class="form-group"><label>Adresse e-mail</label><input type="email" id="r-email" placeholder="votre@email.com" autocomplete="email"></div>
        <div class="form-group"><label>Mot de passe</label><input type="password" id="r-pwd" placeholder="8 caractères minimum" autocomplete="new-password"></div>
        <div class="form-group"><label>Organisation / Fonction</label><input type="text" id="r-org" placeholder="Ex : Journaliste, Chercheur, Administration..."></div>
        <button class="btn-primary" onclick="doRegister()"><i class="fas fa-user-plus"></i> Créer mon compte</button>
...
<div class="modal-overlay hidden" id="modal-admin" onclick="closeOnBg(event,'modal-admin')">
  <div class="modal-box" style="max-width:820px;width:95vw">
    ...
    <div class="modal-body">
      <!-- Login admin si non connecté -->
      <div id="admin-login-pane">
        <p style="font-size:.85rem;color:var(--gray);margin-bottom:14px">Accès restreint — identifiez-vous en tant qu'administrateur.</p>
        <div class="form-group"><label>Mot de passe administrateur</label><input type="password" id="admin-pwd" placeholder="Mot de passe admin" onkeydown="if(event.key==='Enter')doAdminLogin()"></div>
        <button class="btn-primary" onclick="doAdminLogin()"><i class="fas fa-unlock"></i> Accéder</button>
...
═══════════════════════════════════════════ */
const ADMIN_PWD='ITIE2025admin'; // mot de passe admin
let currentUser=null;

function loadSession(){
  try{const s=sessionStorage.getItem('openrbe_user');if(s)currentUser=JSON.parse(s);}catch(e){}
  updateAuthUI();
}
function saveSession(u){currentUser=u;sessionStorage.setItem('openrbe_user',JSON.stringify(u));}
function logout(){currentUser=null;sessionStorage.removeItem('openrbe_user');updateAuthUI();updateLocks();showToast('Vous êtes déconnecté.','warn','fa-sign-out-alt');}

function updateAuthUI(){
  const btn=document.getElementById('btn-login'),txt=document.getElementById('btn-login-txt');
  if(currentUser){
    btn.classList.add('logged');
    txt.textContent=currentUser.prenom+' '+currentUser.nom[0]+'.';
    btn.onclick=()=>{ if(confirm('Se déconnecter ?')) logout(); };
  } else {
...
function updateLocks(){
  ...
  ['lock-top','lock-db','lock-ppe'].forEach(id=>{
    const el=document.getElementById(id);
    if(el) el.classList.toggle('show',locked);
  });
}

function hashPwd(s){
  // Hash simple côté client (pour démo)
...
async function doLogin(){
  ...
  if(!email){highlightInvalid('l-email','Saisissez votre adresse e-mail.');return;}
  if(!validateEmail(email)){highlightInvalid('l-email','Adresse e-mail invalide.');return;}
  if(!pwd){highlightInvalid('l-pwd','Saisissez votre mot de passe.');return;}
  const user=await idbGet('users',email);
  if(!user||user.pwd!==hashPwd(pwd)){msg.className='auth-msg err';msg.textContent='Email ou mot de passe incorrect.';return;}
  saveSession(user);
  closeModal('modal-auth');
  updateAuthUI();
  showToast(`Bienvenue, ${user.prenom} !`,'ok','fa-check-circle');
}
...
async function doRegister(){
  ...
  if(!prenom){highlightInvalid('r-prenom','Saisissez votre prénom.');return;}
  if(!nom){highlightInvalid('r-nom','Saisissez votre nom.');return;}
  if(!email||!validateEmail(email)){highlightInvalid('r-email','Adresse e-mail invalide.');return;}
  if(pwd.length<6){highlightInvalid('r-pwd','Mot de passe trop court (6 caractères min).');return;}
  const exists=await idbGet('users',email);
  if(exists){msg.className='auth-msg err';msg.textContent='Cet email est déjà enregistré.';return;}
  const user={prenom,nom,email,org,pwd:hashPwd(pwd),role:'user',createdAt:new Date().toISOString()};
  await idbPut('users',user);
  saveSession(user);
  closeModal('modal-auth');
  updateAuthUI();
  showToast(`Compte créé ! Bienvenue, ${prenom} !`,'ok','fa-user-check');
}
...
function openAuth(){openModal('modal-auth');}
...
function switchAuth(tab){
  ...
  document.getElementById('auth-title').textContent=tab==='login'?'Connexion':'Inscription';
}
...
function renderCC(nm){
  ...
  <div class="card" style="position:relative">
    ...
    <div id="lock-search" class="lock-overlay ${currentUser?'':'show'}">
      <i class="fas fa-lock"></i><p>Connectez-vous pour exporter les données</p>
      <button onclick="openAuth()">Se connecter</button>
    </div>
...
// Override getExportRows pour prendre en compte la recherche entreprise
const _origGetRows=getExportRows;
function getExportRows(src){
  if(src&&src.startsWith('search_')&&window._searchRows) return window._searchRows;
  if(src==='ppe') return FD.filter(r=>r.pp);
  if(src==='db')  return dbF;
  return compS(FD).slice(0,30);
}
...
/* INIT */
openIDB().then(()=>{
  loadSession();popF();applyF();buildNet();
  addPwdToggle('l-pwd');
  addPwdToggle('r-pwd');
