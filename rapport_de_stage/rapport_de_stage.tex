...
\begin{titlepage}
  \begin{center}
    ...
    \begin{minipage}{0.45\textwidth}
      \centering
      {\color{ensaeblue}\bfseries\large ENSAE}\\[0.1cm]
    ...
    {\color{ensaeblue}\LARGE\bfseries Appui à la gestion des données du secteur extractif :
    fiabilisation du Registre des Bénéficiaires Effectifs et analyse
    ...
    \begin{tabular}{p{0.46\textwidth}p{0.46\textwidth}}
      \textbf{Présenté par :} & \textbf{Encadrement :}\\[0.15cm]
      Sié Rachid TRAORÉ & M. Thaddée Adiouma SECK\\
      Élève Ingénieur Statisticien & \emph{Secrétaire Permanent du CN-ITIE}\\
      Économiste (ISE3) & M. Fallou DIONE\\
      ENSAE Pierre Ndiaye, Dakar & \emph{Gestionnaire de données, encadrant}\\
    \end{tabular}\\[1.2cm]
...
Mes remerciements s'adressent en premier lieu à Monsieur \textbf{Thaddée Adiouma
...
Je remercie tout particulièrement Monsieur \textbf{Fallou DIONE}, Gestionnaire de
données et encadrant du stage, pour son suivi hebdomadaire rigoureux, ses
...
\textbf{Rasmané BAMOGO}, avec qui les travaux communs sur le Registre des
...
\begin{itemize}
  \item \textbf{fiabiliser et enrichir le Registre des Bénéficiaires Effectifs
  (RBE)} des sociétés extractives, en assurant une conformité renforcée avec
  l'exigence 2.5 de la Norme ITIE 2023 ;
  \item \textbf{produire une base analytique robuste sur les Exploitations
...
menés sur le Registre des Bénéficiaires Effectifs : audit de qualité des
données 2021--2025 et développement de la plateforme de consultation
...
\begin{tabular}{p{4.5cm}p{9.5cm}}
  ...
  Encadrement technique & M. Fallou DIONE, Gestionnaire de données\\
...
analyse et modélisation (S7--S9), livrables finaux (S10--S12). Les travaux qui
...
réglementaire et méthodologique : Norme ITIE 2023 (exigence 2.5), décret
n°~2020-791 et ses modificatifs, décret n°~2025-1354, arrêté ministériel
n°~1598 relatif au formulaire de déclaration des bénéficiaires effectifs,
codes minier et pétrolier, ainsi que le diagnostic OE 2024 et le projet d'étude
« Bénéficiaires effectifs » du Sénégal.

La collecte a ensuite mobilisé plusieurs sources croisées : la base RBE
...
cinq exercices (2021--2025) et recense \textbf{628 enregistrements} de
bénéficiaires effectifs pour \textbf{317 entreprises} réparties dans neuf
régions. Les principaux constats sont les suivants :

\begin{itemize}
  \item \textbf{Complétude} : sur 7~536 cellules attendues, 23~\% sont
  ...
  Exposée concentrent les déficits les plus graves : « Nom PPE » absent dans
  97,1~\% des cas, « Est une PPE » dans 85,2~\%. Les variables financières
  sont également lacunaires (27,1~\% des pourcentages d'actions et 41,9~\% des
  droits de vote manquants) ;
  \item \textbf{Doublons} : 12 enregistrements en double (1,9~\% des
  ...
  \item \textbf{Erreurs de saisie} : 547 enregistrements affectés, dont 545
  contenant un code binaire dans le champ textuel « Fonction PPE » et 14 un nom
  de pays dans le champ « Est une PPE » ;
  \item \textbf{Cohérences structurelles} : entreprises dont la somme des
  ...
  \item \textbf{Couverture} : analyse croisée des statuts de déclaration par
...
adressées au CN-ITIE : fiabilisation du formulaire de déclaration, contrôles de
saisie bloquants, priorité de relance sur les variables PPE et financières, et
dédoublonnage systématique. Il conditionne aussi l'analyse des risques prévue
par le programme OE (âges atypiques, croisements GAFI/UE) : celle-ci n'a pu
...
la forme d'\textbf{OpenRBE}, une application de consultation et de valorisation
du RBE développée de manière itérative (versions v1.0 à v7, puis versions web
successives et version de livraison), en concertation avec l'encadrant dont les
observations étaient intégrées à chaque itération.

Deux déclinaisons complémentaires ont été produites :

\begin{itemize}
  \item une \textbf{application R Shiny} modulaire (modules tableau de bord,
  recherche, cartographie, réseau, PPE) avec bibliothèque d'indicateurs et de
  graphiques ;
  \item une \textbf{application web autonome} générée par un script Python
...
institutionnel et statistique du sous-secteur, et estimer les volumes d'or
...
\begin{itemize}
  \item un \textbf{écart cumulé de 25,4 tonnes} entre les importations d'or
  ...
  \item un \textbf{taux moyen de non-déclaration de 27,1~\%} sur la période,
  avec une forte variabilité annuelle (50,4~\% en 2020, 4,6~\% en 2024) dont
  l'interprétation requiert prudence ;
  \item une \textbf{production EMAPE déclarée dérisoire} : 62~kg en 2023,
  ...
  majeur ;
  \item une \textbf{concentration géographique persistante} des importations
  ...
  \item une \textbf{triangulation} des estimations miroir avec les données de
...
La cartographie des circuits de commercialisation a été établie : structure des
acheteurs de la production artisanale (comptoirs d'achat, collecteurs,
négociants, exportateurs), corridors d'exportation et acteurs de la chaîne de
valeur, avec identification des obstacles institutionnels, techniques,
...
extraites et consolidées en bases exploitables : production et exportations
par substance, flux financiers, paiements sociaux, emplois, contribution du
secteur extractif au budget de l'État, données hydrocarbures, complétées par
...
RBE au regard du nouveau décret n°~2025-1354.
...
\begin{itemize}
  \item les livrables analytiques majeurs sont \textbf{réalisés} : rapport
  final d'audit de la qualité du RBE 2021--2025, rapport analytique EMAPE
  (statistique miroir 2020--2024), note méthodologique sur l'analyse miroir,
  inventaire des sociétés sans déclaration, plateforme de consultation OpenRBE
  et supports de restitution ;
  \item plusieurs chantiers sont \textbf{partiellement réalisés} et
  constituent des prolongements naturels : formalisation du schéma relationnel
  à trois tables avec identifiants uniques, enrichissement ISIN/cotation,
  analyse de risques GAFI/UE (limitée par la carence des variables PPE),
  cartographie interactive des sites EMAPE, détection systématique des
  structures d'actionnariat opaques ;
  \item les \textbf{enquêtes de terrain} à Kédougou n'ont pas pu être menées
...
\textbf{qualité des données sources} : la carence quasi totale des variables
...
l'\textbf{accessibilité des données} : certaines sources externes (cotations,
données douanières fines, données de terrain) n'étaient pas mobilisables dans
les délais. Enfin, la \textbf{contrainte temporelle} d'un stage de douze
...
\begin{itemize}
  \item des compétences \textbf{statistiques et économétriques} appliquées :
  audit de qualité de données, statistique miroir, triangulation de sources,
  analyse de flux commerciaux ;
  \item des compétences \textbf{techniques} : R (traitement de données,
  R Markdown, Shiny), Python (scripts de construction de la plateforme,
  structuration de fichiers), Excel avancé, QGIS, visualisation de réseaux,
  \LaTeX{} ;
  \item des compétences \textbf{sectorielles} : Norme ITIE 2023, propriété
  effective et lutte anti-blanchiment (GAFI, PPE), gouvernance minière et
  pétrolière sénégalaise, fiscalité extractive et modélisation FARI ;
  \item des compétences \textbf{professionnelles} : travail en binôme et en
  équipe multipartite, restitution devant des non-statisticiens, gestion de
  livrables contractuels sous contrainte de confidentialité.
\end{itemize}
...
\begin{enumerate}
  \item \textbf{Fiabiliser la chaîne de collecte du RBE} : formulaire à
  contrôles bloquants, champs PPE obligatoires, dédoublonnage à la saisie et
  identifiants uniques partagés avec le RCCM, la DGI et Landfolio ;
  \item \textbf{Pérenniser et institutionnaliser OpenRBE} : hébergement,
  gestion des utilisateurs et des exports, mise à jour administrée des données,
  formalisation du schéma Entités--Personnes--Relations pour le calcul des
  participations indirectes ;
  \item \textbf{Systématiser l'analyse miroir} comme outil de veille annuelle
  sur les flux d'or, en lien avec les douanes, l'ANSD et la BCEAO ;
  \item \textbf{Renforcer la statistique du sous-secteur EMAPE} : base
  centralisée des comptoirs et des déclarations de production, couplage avec
  les données satellitaires et les titres Landfolio, enquêtes de terrain
  régulières à Kédougou ;
  \item \textbf{Poursuivre la montée en charge sur la modélisation
...
concrets : un diagnostic chiffré et actionnable de la qualité du Registre des
...
statisticien économiste dans une institution de transparence : transformer des
...
\begin{longtable}{p{5.2cm}p{2.1cm}p{6.6cm}}
  ...
  Intégration des observations et finalisation & Oui & Versions successives des rapports et de la plateforme intégrant les observations de l'encadrant.\\
  \sousentete \multicolumn{3}{l}{\textbf{Travaux complémentaires (hors cahier des charges)}}\\
